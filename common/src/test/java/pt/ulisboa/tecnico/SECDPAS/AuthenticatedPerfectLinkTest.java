...
public class AuthenticatedPerfectLinkTest {
    ...
    public void successRegisterSignatureCompromise(){
        ...
        byte[] freshness = new byte[0];
        byte[] signature = SignatureHandler.publicSign(freshness, privServer);
        byte[] incorrectSignature = SignatureHandler.publicSign(Longs.toByteArray(1), privServer);

        Contract.ACK incorrectResponse = Contract.ACK.newBuilder().setFreshness(ByteString.copyFrom(new byte[0])).setSignature(ByteString.copyFrom(incorrectSignature)).build();
